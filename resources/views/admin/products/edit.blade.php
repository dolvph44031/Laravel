                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="product-title-input"
                                    placeholder="Tên sản phẩm" name="name" value="{{ old('name', $product->name) }}">
                            </div>
                            <!-- Product Price -->
                            <div class="mb-3">
                                <label for="product-price-input" class="form-label">Giá</label>
                                <input type="text" class="form-control" id="product-price-input"
                                    placeholder="Giá sản phẩm" name="price" value="{{ old('price', $product->price) }}">
                            </div>
                            <!-- Product Sale Price -->
                            <div class="mb-3">
                                <label for="product-sale-price-input" class="form-label">Giá sale</label>
                                <input type="text" class="form-control" id="product-sale-price-input"
                                    placeholder="Giá giảm" name="price_sale"
                                ...
                                @foreach ($product->galleries as $gallery)
                                    <div class="d-inline-block position-relative me-2">
                                        <img src="{{ Storage::url($gallery->image) }}" width="70px" height="70px"
                                            class="rounded">
                                        <!-- Delete button -->
                                        <form action="{{ route('admin.product-galleries.destroy', $gallery) }}" method="POST" class="position-absolute top-0 end-0 m-1">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    <tbody>
                                        @foreach ($product->variants as $variant)
                                            <tr>
                                                ...
                                                <input type="hidden" name="product_variants[{{ $loop->index }}][id]"
                                                    value="{{ $variant->id }}">

                                                <!-- Size Selection -->
                                                <td>
                                                    <select name="product_variants[{{ $loop->index }}][size]"
                                                        class="form-control">
                                                        @foreach ($sizes as $size_id => $size_name)
                                                            <option value="{{ $size_id }}"
                                                                {{ $variant->product_size_id == $size_id ? 'selected' : '' }}>
                                                                {{ $size_name }}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                ...
                                                <!-- Color Selection -->
                                                <td>
                                                    <select name="product_variants[{{ $loop->index }}][color]"
                                                        class="form-control">
                                                        @foreach ($colors as $color_id => $color_name)
                                                            <option value="{{ $color_id }}"
                                                                {{ $variant->product_color_id == $color_id ? 'selected' : '' }}>
                                                                {{ $color_name }}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                ...
                                                <td>
                                                    <input type="file"
                                                        name="product_variants[{{ $loop->index }}][image]"
                                                        class="form-control">
                                                    @if ($variant->image)
                                                ...
                                                <td>
                                                    <input type="text"
                                                        name="product_variants[{{ $loop->index }}][quantity]"
                                                ...
                                                <td>
                                                    <input type="text"
                                                        name="product_variants[{{ $loop->index }}][price]"
                    ...
                    <div class="collapse show" id="collapseStatus">
                        <div class="card-body">
                            ...
                            <select class="form-control form-select-lg mb-3" id="choices-publish-status-input"
                                ...
                                <option value="1" {{ old('status', $product->is_active) == 1 ? 'selected' : '' }}>
                                    Hoạt động</option>
                                <option value="0" {{ old('status', $product->is_active) == 0 ? 'selected' : '' }}>
